...
def td3(env_fn: Callable,
    ...
    def compute_loss_q(data):
        ...
        with torch.no_grad():
            ...
            o2_action= torch.ones(a2.size(),device=torch.device('cuda'))
            for i in range (a2.size()[0]):
                if int(o2[i][24].tolist())== -1:
                    a[3],a[7],a[11]=torch.tensor([0,0,0],device=torch.device('cuda'))
                    o2_action[i][0],o2_action[i][4],o2_action[i][8]=torch.tensor([0,0,0],device=torch.device('cuda'))
                if int(o2[i][25].tolist())== -1:
                    o2_action[i][1],o2_action[i][5],o2_action[i][9]=torch.tensor([0,0,0],device=torch.device('cuda'))
                if int(o2[i][26].tolist())== -1:
                    o2_action[i][2],o2_action[i][6],o2_action[i][10]=torch.tensor([0,0,0],device=torch.device('cuda'))
                if int(o2[i][27].tolist())== -1:
                    o2_action[i][3],o2_action[i][7],o2_action[i][11]=torch.tensor([0,0,0],device=torch.device('cuda'))
            a2 = a2 *o2_action
            # Target Q-values
